...
export function MissionComponent({ selectedPlayerId, haitacConfig, onConfigChange }: MissionComponentProps): React.JSX.Element {
  ...
  const handleStartPullPirate = () => {
    if (!fixedTeam.trim()) {
      console.error('Please enter fixed team (leader,member1,member2,member3)');
      return;
    }
    ...
    if (teamIds.length !== 4) {
      console.error('Fixed team must have exactly 4 members (leader,member1,member2,member3)');
      return;
    }

    const [leaderId, member1Id, member2Id, member3Id] = teamIds;

    console.log(`Starting Kéo Hải Tặc: Members [${member1Id}, ${member2Id}, ${member3Id}] inviting leader ${leaderId}`);
  ...
  const handleStartPirate = () => {
    if (!rotatingIds.trim()) {
      console.error('Please enter rotating member IDs');
      return;
    }

    if (!fixedTeam.trim()) {
      console.error('Please enter fixed team');
      return;
    }
    ...
    if (ids.length === 0) {
      console.error('No valid rotating member IDs');
      return;
    }
    ...
    if (teamIds.length !== 4) {
      console.error('Fixed team must have exactly 4 members');
      return;
    }
    ...
    console.log(`Starting Hải Tặc: Inviting first rotating member ${ids[0]} to leader ${leaderId}`);
